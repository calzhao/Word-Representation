...
# Data Parameter
vocabulary_size=200  #预定义频繁单词库的长度
...
# 建立样本集
train_len=(int)(4*len(raw_text)/5)-10
train_sentences =raw_text[:train_len]
...
outputs, states = tf.nn.dynamic_rnn(cell, embeddings, dtype=tf.float32)
...
np.savetxt('RNN_embeddings.txt', trained_embeddings, fmt="%.5f", delimiter=' ') #保存为2位小数的浮点数，用逗号分隔
...
plot_with_labels(low_dim_embs, labels, './RNN_embeddings.png')
print('画图完成')